...
export const Signup = () => {
  ...
  // Redirect when user AND profile are both ready
  useEffect(() => {
    if (user && profile) {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Write Firestore profile with ALL required fields so AuthContext doesn't race
      ...
      // AuthContext will pick up the user + profile and the useEffect above redirects
    } catch (err: any) {
  ...
  const handleGoogleSignup = async () => {
    ...
    try {
      ...
      // AuthContext handles profile creation + redirect via useEffect
    } catch (error: any) {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      >
        ...
        <p className="mt-2 text-center text-sm text-gray-600">
          Join Shree Lakshmi Child Hospital
        </p>
      </motion.div>
      ...
      >
        <div className="bg-white py-10 px-4 shadow-xl sm:rounded-2xl sm:px-10 border border-gray-100">
          ...
          <p className="mt-8 text-center text-sm text-gray-600">
            ...
            <Link to={`/login?returnUrl=${encodeURIComponent(returnUrl)}`} className="font-semibold text-emerald-600 hover:text-emerald-500">
              Sign in
            </Link>
          </p>
        </div>
